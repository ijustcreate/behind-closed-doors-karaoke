(function () {
  ...
  function renderMenu() {
    const menu = current(), page = document.querySelector('.menuPage'), stage = document.getElementById('menuStage'), canEdit = !!currentUser()?.isAdmin; if (!page) return;
    ...
    const controls = canEdit ? `<div class="menuTopControls"><select id="savedMenuSelect" class="menuSelect" onchange="selectDrinkMenu(this.value)" aria-label="${t[2]}"></select><button id="menuEditBtn" class="btn gold small" onclick="toggleMenuEdit()">${edit ? t[1] : t[0]}</button></div>` : '';
    page.innerHTML = `${controls}<div class="menuPageHeader${canEdit ? '' : ' noMenuControls'}"><div class="menuKey">⚿</div><h2>Behind Closed Doors Bar</h2></div>${special}<div class="drinkList">${drinks}</div>${edit ? `<button class="addDrinkButton" onclick="addDrink()">${t[5]}</button>` : ''}<p class="menuDisclaimer">Digital menu may not be accurate.<br><strong>The real menu at the bar is always right.</strong></p>`;
    renderMenuLibrary();
  }

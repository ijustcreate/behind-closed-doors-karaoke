(function () {
  ...
  window.sendChat = event => {
    ...
    sharedFetch('karaoke_chat_messages', { method: 'POST', body: JSON.stringify({ id: row.id, profile_id: row.profileId, singer_name: row.singerName, message: row.message, night_key: nightKey(), image_urls: row.images }) }).then(syncChat).catch(() => toast('Message saved here; shared chat is reconnecting'));
  };
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    const intro = document.querySelector('.menuIntro'); if (intro) { intro.querySelector('h2').textContent = 'Drink Menu'; intro.querySelector('p').textContent = 'Tap the leather cover to reveal the menu, then browse the drinks behind closed doors.'; document.getElementById('menuEditBtn')?.remove(); }

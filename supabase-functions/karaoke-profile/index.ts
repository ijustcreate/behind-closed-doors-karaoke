...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(body.action==="list_profiles"){const actor=await adminActor();if(actor==="password_required")return json({error:"Add a password to your admin profile before managing staff accounts."},403);if(!actor)return json({error:"Admin authorization required"},403);const {data,error}=await admin.from("karaoke_profiles").select("id,username,display_name,is_admin,last_seen").order("display_name");if(error)throw error;return json({profiles:data.map(shape)});}
  ...
  const entered=String(body.input||"").trim();if(!entered||entered.length>40)return json({error:"Enter a preferred name"},400);
  const {data:matches,error:lookupError}=await admin.from("karaoke_profiles").select("id,username,display_name,password_hash,is_admin,last_seen").or(`username.ilike.${entered},display_name.ilike.${entered}`).limit(1);if(lookupError)throw lookupError;

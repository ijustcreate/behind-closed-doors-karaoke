(function () {
  ...
  async function menuSettingAction(action,menu){
    if(action==='set_active_menu'){
      const marker=`[[BCD_CHATBOT:${state.chatbotEnabled===false?'OFF':'ON'}]]`,subheader=String(menu?.subheader||'').replace(/^\[\[BCD_CHATBOT:(?:ON|OFF)\]\]\s*/,'');
      return adminProfileAction(action,{menu:{...menu,subheader:marker+subheader}});
    }
  ...
  function ensurePersistentMenuPicker(){
    const select=document.getElementById('savedMenuSelect'),controls=select?.closest('.menuTopControls')||select?.parentElement;if(!select||!controls)return;
    select.hidden=true;
    const admin=!!currentUser()?.isAdmin;
    document.body.classList.toggle('menu-admin',admin);
  ...
  window.syncSharedDrinkMenu=async()=>{
    ...
    try{const result=await menuSettingAction('get_active_menu'),menu=result.menu,updatedAt=Date.parse(result.updatedAt||'');if(!menu?.name||!Array.isArray(menu.drinks)||result.updatedAt===sharedMenuStamp){ensurePersistentMenuPicker();return}if(typeof menu.subheader==='string')menu.subheader=menu.subheader.replace(/^\[\[BCD_CHATBOT:(?:ON|OFF)\]\]\s*/,'');if(Number.isFinite(updatedAt)&&updatedAt<BCD_CURRENT_MENU_PUBLISHED_AT){sharedMenuStamp=result.updatedAt;ensurePersistentMenuPicker();return}sharedMenuStamp=result.updatedAt||JSON.stringify(menu);state.drinkMenus||={};state.drinkMenus[menu.name]=menu;state.activeDrinkMenu=menu.name;saveState();renderDrinkMenu();renderMenuLibrary();ensurePersistentMenuPicker()}catch(error){console.warn('Live drink menu unavailable',error)}
  };
  ...
  const nativeRememberMenuAccess=window.rememberSharedProfile;
  if(typeof nativeRememberMenuAccess==='function')window.rememberSharedProfile=function(){const result=nativeRememberMenuAccess.apply(this,arguments);requestAnimationFrame(enforceMenuAccess);return result};
  const nativeLogoutMenuAccess=window.logout;
  if(typeof nativeLogoutMenuAccess==='function')window.logout=function(){const result=nativeLogoutMenuAccess.apply(this,arguments);requestAnimationFrame(enforceMenuAccess);return result};
  ...
  window.addEventListener('DOMContentLoaded',()=>{
    ...
    let repairedDrinkEncoding=false;
    Object.values(state.drinkMenus||{}).forEach(menu=>(menu.drinks||[]).forEach(drink=>{
      ['name','description'].forEach(key=>{if(typeof drink[key]!=='string')return;const fixed=drink[key].replaceAll('JalapeÃ±o','Jalapeño').replaceAll('JALAPEÃ±O','JALAPEÑO');if(fixed!==drink[key]){drink[key]=fixed;repairedDrinkEncoding=true}})
    }));
    if(repairedDrinkEncoding){saveState();renderDrinkMenu();renderMenuLibrary()}
